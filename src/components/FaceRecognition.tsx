      ...
      <div className="lg:col-span-2 bg-white rounded-lg shadow">
        ...
        <div className="p-6">
          ...
          {status === 'enrollment' && (
            <div className="mt-4 bg-blue-50 border border-blue-200 rounded-lg p-4">
              <p className="text-sm text-blue-800 font-medium mb-2">
                Register test users from webcam
              </p>
              <p className="text-sm text-blue-700 mb-4">
                {enrollmentMessage ??
                  'Reference photos were not found. Capture each person while facing the camera.'}
              </p>
          ...
          <p className="mt-4 text-sm text-gray-500 text-center">
            Add photos to `public/faces/` for automatic setup, or capture each
            person from the webcam when files are missing.
          </p>
        </div>
      </div>
        ...
        <div className="bg-white rounded-lg shadow">
          ...
          <div className="p-6 space-y-3">
            {FACE_PROFILES.map((profile) => {
              const isRegistered = registeredNames.includes(profile.name);
              const isPending = pendingProfiles.some(
              ...
              return (
                ...
                >
                  <div>
                    ...
                    <p className="text-xs text-gray-500">{profile.imagePath}</p>
                  </div>
                  <span
